...
// ─── component ────────────────────────────────────────────────────────────────
// Accepts both prop-naming conventions:
//   • user portal:   selectedProduct / closeModal / setToastMsg
//   • public pages:  product         / onClose    / onAddToCart
export default function ProductModal(props) {
    // normalise prop names
    const p          = props.selectedProduct ?? props.product ?? null;
    const closeModal = props.closeModal      ?? props.onClose ?? (() => {});
    const onAddToCart= props.onAddToCart     ?? null;
    ...
    const handleAddToCart = async () => {
        ...
        if (onAddToCart) {
            onAddToCart(p, qty);
        } else if (auth.addToCart) {
            await auth.addToCart(p.id, qty);
            props.setToastMsg?.(`Added ${qty} item(s) to Cart`);
            setTimeout(() => props.setToastMsg?.(''), 3000);
        }
        closeModal();
    };
