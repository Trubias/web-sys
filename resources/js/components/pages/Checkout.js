                ...
                <div className="mobile-stack" style={{ display: 'flex', gap: '4rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                    ...
                    <form onSubmit={handleSubmit} style={{ flex: '1 1 600px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                            ...
                            <button type="button" onClick={() => navigate('/user/cart')} disabled={isSubmitting} style={{ flex: 1, padding: '1rem', background: 'transparent', color: '#666', fontWeight: 800, textTransform: 'uppercase', fontSize: '1.1rem', border: '1px solid #ccc', borderRadius: '4px', cursor: isSubmitting ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }} onMouseEnter={e => { if (!isSubmitting) e.target.style.background = '#f5f5f5'; e.target.style.color = '#333'; }} onMouseLeave={e => { if (!isSubmitting) e.target.style.background = 'transparent'; e.target.style.color = '#666'; }}>
                                Cancel
                            </button>
                        </div>
                    </form>
